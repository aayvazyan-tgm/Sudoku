...
int puzzle_is_valid(value_t puzzle[psize][psize]) {
	...
	if(xsudoku==1){
		...
		for(vertikal=0;vertikal<psize;vertikal++){
			...
			vorkommnisse[vertikal]=puzzle[row][col];

			horizontal++;

		}

		//now checks if a value is stored twice in vorkommnisse
		...
		for(i=0;i<psize;i++){
			...
			for(j=0;j<psize;j++){
				...
				if(vorkommnisse[i]==vorkommnisse[j])return 0;
	...
	//Checks the diagonal values starting from the buttom left
